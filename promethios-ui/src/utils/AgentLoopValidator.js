...
const testCases = [
  ...
  { agentId: 'Core.Forge', prompt: 'Hello.', followUp: 'What\'s your name?', expectedBehavior: 'Identity + memory logging' },
  { agentId: 'OpsAgent', prompt: 'Scaffold a vertical.', expectedBehavior: 'Task delegation' },
  { agentId: 'ObserverAgent', prompt: 'Summarize what just happened.', expectedBehavior: 'Contextual reflection' },
  ...
  // Life Tree Agent
  { agentId: 'LifeTree', prompt: 'Prompt a reflection about my childhood.', expectedBehavior: 'Reflective journaling prompt' },
  
  // Site Plan Agent
  { agentId: 'SiteGen', prompt: 'Generate a draft site plan for 2 acres.', expectedBehavior: 'Returns layout/config draft' },
  
  // NEUREAL Agent
  { agentId: 'NEUREAL', prompt: 'Splice a dream based on today\'s emotions.', expectedBehavior: 'Surreal or symbolic output' }
];
...
function AgentLoopValidator() {
  ...
  const runTest = async (test) => {
    ...
    try {
      ...
      const memoryLogged = memoryData.some(entry => 
        entry.agent_id === test.agentId && 
        entry.content.includes(test.prompt)
      );
      ...
      if (test.followUp) {
        ...
        const contextual = followUpData.message && 

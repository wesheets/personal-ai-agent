...
const AgentChat: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (resolvedAgentId.toLowerCase() === 'hal' && input.toLowerCase().includes('onboarding')) {
    ...
    const makeRequest = async (isRetry = false) => {
      try {
        ...
        if (streaming) {
          ...
          setMessages((prev) =>
            prev.map((msg) =>
              msg.id === assistantMessageId ? { role: 'assistant', content: responseText } : msg
            )
          );
        } else {
        ...
        if (error.message === 'UNAVAILABLE' || error.name === 'AbortError') {
          setMessages((prev) => [
            ...
            {
              role: 'system',
              content: `⚠️ This agent is temporarily unavailable. Please try again or switch agents.`
            }
          ]);
        } else {
          setMessages((prev) => [
  ...
  const renderInstructionCard = (instruction: Instruction, isExpanded: boolean, index: number) => {
    ...
    return (
      <Box mt={2} p={3} bg={bgColor} borderRadius="md" borderWidth="1px" borderColor={borderColor}>
        <Flex
  ...
  return (
    <Flex flexDirection="column" height="100vh">
      <Flex p={2} justifyContent="space-between" alignItems="center" borderBottomWidth="1px">
        ...
        <Flex gap={2}>
          <Button onClick={clearHistory} size="sm" colorScheme="gray">
            Clear History
          </Button>
      ...
      <Flex p={4} borderTopWidth="1px" alignItems="center" gap={2}>
        <Button onClick={() => setDebugOpen(!debugOpen)} size="sm" variant="outline">
          &lt;/&gt;
        </Button>
        <Button onClick={() => setStreaming(!streaming)} size="sm" variant="outline">
        ...
        />
        <Button onClick={handleSubmit} isDisabled={loading} colorScheme={loading ? 'gray' : 'blue'}>
          {loading ? (
            <Flex alignItems="center">

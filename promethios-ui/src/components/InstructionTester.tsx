...
import { Instruction, OrchestratorInstructions, InstructionExpectedOutput } from '../../backend/instruction_engine';
...
const InstructionTester: React.FC = () => {
  ...
  const checkValidationStatus = () => {
    ...
    const checkpointsValid = checkpointService.areAllRequiredCheckpointsApproved(createdInstruction);
  ...
  return (
    <Box p={5} borderWidth="1px" borderRadius="lg" maxWidth="800px" mx="auto">
      <Heading size="lg" mb={4}>Instruction Schema Engine Tester</Heading>
      
      <VStack spacing={6} align="stretch">
        {/* Instruction Creation Form */}
        <Box>
          <Heading size="md" mb={3}>Create Test Instruction</Heading>
          ...
          <VStack spacing={4} align="stretch">
            ...
            <FormControl>
              ...
              <HStack>
                ...
                />
                <Button leftIcon={<AddIcon />} onClick={addTool}>Add</Button>
              </HStack>
              <Box mt={2}>
                {toolsRequired.map((tool) => (
                  <Tag key={tool} size="md" borderRadius="full" variant="solid" colorScheme="blue" m={1}>
            ...
            <FormControl>
              ...
              <HStack>
                ...
                />
                <Checkbox isChecked={outputRequired} onChange={(e) => setOutputRequired(e.target.checked)}>
                  Required
                </Checkbox>
                <Button leftIcon={<AddIcon />} onClick={addExpectedOutput}>Add</Button>
              </HStack>
              <Box mt={2}>
                {expectedOutputs.map((output, index) => (
                  <Tag 
                    ...
                    m={1}
                  >
                    <TagLabel>{output.type}: {output.tag}</TagLabel>
            ...
            <HStack>
              ...
              <FormControl>
                ...
                <VStack align="start">
                  <Checkbox isChecked={allowRetry} onChange={(e) => setAllowRetry(e.target.checked)}>
                    Allow Retry
                  </Checkbox>
                  <Checkbox isChecked={escalateOnFailure} onChange={(e) => setEscalateOnFailure(e.target.checked)}>
                    Escalate on Failure
                  </Checkbox>
                </VStack>
              </FormControl>
        ...
        {createdInstruction && (
          <Box>
            <Heading size="md" mb={3}>Test Instruction</Heading>
            ...
            <VStack mt={4} spacing={4} align="stretch">
              <Box>
                <Heading size="sm" mb={2}>Complete Outputs</Heading>
                <VStack align="stretch">
                  {createdInstruction.expected_outputs.map((output, index) => (
                    <Button 
                      key={index}
                      onClick={() => completeOutput(output.type, output.tag)}
                      isDisabled={completedOutputs.some(o => o.type === output.type && o.tag === output.tag)}
                      colorScheme="green"
                      size="sm"
                    >
              ...
              <Box>
                <Heading size="sm" mb={2}>Simulate Tool Usage</Heading>
                <HStack>
                  <Input 
                    value={usedTool} 
              ...
              <Box>
                <Heading size="sm" mb={2}>Loop Control</Heading>
                <HStack>

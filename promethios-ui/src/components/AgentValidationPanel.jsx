    ...
    <div className="p-4 max-w-6xl mx-auto">
      ...
      <div className="mb-6 bg-gray-100 p-4 rounded-lg">
        ...
        {validator.currentTest && (
          <div className="bg-blue-50 p-3 rounded border border-blue-200 mb-4">
            <p className="font-medium">Currently testing: <span className="font-bold">{validator.currentTest.agentId}</span></p>
          ...
          {validator.logEntries.length === 0 ? (
            ...
            validator.logEntries.map((log, index) => (
              <div key={index} className={`mb-1 ${
                ...
                {log.details && <div className="pl-6 text-xs opacity-80 mt-1 mb-2">{log.details}</div>}
              </div>
            ))
          )}
        </div>
                ...
                <>
                  <div className="grid grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="bg-green-100 p-3 rounded">
                      ...
                      <p className="text-2xl font-bold text-green-700">{report.summary.passedTests}</p>
                    </div>
                    <div className="bg-red-100 p-3 rounded">
                      <p className="text-sm text-gray-600">Failed</p>
                      <p className="text-2xl font-bold text-red-700">{report.summary.failedTests}</p>
                    </div>
                    <div className="bg-blue-100 p-3 rounded">
                      <p className="text-sm text-gray-600">Success Rate</p>
                      <p className="text-2xl font-bold text-blue-700">{report.summary.successRate}%</p>
                    </div>
                  </div>
                          ...
                          <tr key={index} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                            ...
                            <td className="py-2 px-4">
                              {result.memoryLogged !== undefined && (
                                <span className={`inline-block px-2 py-1 rounded text-xs font-medium ${
                                  result.memoryLogged ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                                }`}>
      ...
      <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
        ...
        <ul className="list-disc pl-5 space-y-1">
          ...
          <li>Delegation events like:
            <pre className="bg-gray-100 p-2 rounded mt-1 text-xs">LOG: Core.Forge delegated task to OpsAgent</pre>
            <pre className="bg-gray-100 p-2 rounded mt-1 text-xs">LOG: Ash executed analysis on Core.Forge</pre>
          </li>
        </ul>
      </div>
    </div>

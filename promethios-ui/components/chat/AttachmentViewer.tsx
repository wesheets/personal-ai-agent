...
const AttachmentViewer: React.FC<AttachmentViewerProps> = ({ attachments }) => {
  ...
  const renderAttachmentContent = (attachment: Attachment) => {
    ...
    switch (attachment.type) {
      case 'image':
        return <Text>Image: {attachment.name}</Text>;
      case 'tool_result':
      case 'memory_block':
        return <Text>Tool result: {attachment.name}</Text>;
      case 'file':
      default:
        return <Text>File: {attachment.name}</Text>;
    }
  };
  ...
  return (
    <Box mt={2}>
      {attachments.map((attachment, index) => (
        <MessageAttachment key={index} attachment={attachment} onOpen={handleOpen} />
      ))}
      ...
      {isOpen && selectedAttachment && (
        ...
        >
          ...
          <Box as="button" mt={2} onClick={() => setIsOpen(false)} fontSize="sm" color="blue.500">
            Close
          </Box>
        </Box>
      )}

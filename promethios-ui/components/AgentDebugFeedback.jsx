...
const AgentDebugFeedback = ({ 
  ...
  const hasLifecycleEvents = lifecycleEvents && typeof lifecycleEvents === 'object' && Object.keys(lifecycleEvents).length > 0;
  
  // Check if we have performance metrics
  const hasPerformanceMetrics = performanceMetrics && typeof performanceMetrics === 'object' && Object.keys(performanceMetrics).length > 0;
  
  // Check if we have API duration data
  const hasApiDuration = hasLifecycleEvents && 
  ...
  const hasTotalDuration = hasLifecycleEvents && 
            ...
            <VStack align="stretch" spacing={3}>
              ...
              <Box>
                <HStack justifyContent="space-between" mb={1}>
                  <Text fontSize="sm" fontWeight="bold">Debug Logs</Text>
                  <HStack>
                    <Box>
                      <select
                        value={logFilter}
                  ...
                  {hasLogs ? (
                    filteredLogs.map((log, index) => (
                      <Text 
                        ...
                        mb={1}
                        color={
                          log?.includes('❌') || log?.includes('error') ? 'red.500' :
                          log?.includes('⚠️') || log?.includes('warning') ? 'orange.500' :
                          log?.includes('✅') || log?.includes('success') ? 'green.500' :
                          undefined
                        }
                      >
                        {log ?? ''}
              ...
              <Box>
                <Text fontSize="sm" fontWeight="bold" mb={1}>Lifecycle Events</Text>
                <Box 
                  bg={colorMode === 'light' ? 'gray.100' : 'gray.800'} 
                  p={2} 
                  borderRadius="md"
                >
                  {hasLifecycleEvents ? (
                    <VStack align="stretch" spacing={1}>
                      ...
                      {hasApiDuration && (
                        <HStack justifyContent="space-between" mt={1} pt={1} borderTopWidth="1px" borderTopColor={colorMode === 'light' ? 'gray.200' : 'gray.600'}>
                          <Text fontSize="xs" fontFamily="monospace" fontWeight="bold">
                            API Duration:
                          </Text>
                          <Text fontSize="xs" fontFamily="monospace" fontWeight="bold">
                            {calculateDuration(lifecycleEvents.apiCallStart, lifecycleEvents.apiResponse)}
                          </Text>
                        </HStack>
                      )}
                      ...
                      {hasTotalDuration && (
                        <HStack justifyContent="space-between">
                          ...
                          <Text fontSize="xs" fontFamily="monospace" fontWeight="bold">
                            {calculateDuration(lifecycleEvents.submitClick, lifecycleEvents.spinnerResetComplete)}
                          </Text>
                        </HStack>
                      )}
                    </VStack>
              ...
              <Box>
                <Text fontSize="sm" fontWeight="bold" mb={1}>Performance Metrics</Text>
                <Box 
                  bg={colorMode === 'light' ? 'gray.100' : 'gray.800'} 
                  p={2} 
                  borderRadius="md"
                      ...
                      {Object.entries(performanceMetrics).map(([key, value]) => (
                        <HStack key={`metric-${key}`} justifyContent="space-between">
                          ...
                          <Text 
                            ...
                            fontFamily="monospace"
                            color={
                              value === 'true' ? 'green.500' :
                              value === 'false' ? 'gray.500' :
                              value === 'Error' ? 'red.500' :
                              undefined
                            }
                          >

...
const MemoryViewer = () => {
  ...
  const filteredMemories = viewMode === 'raw' ? memories : memories.filter(mem => {
  ...
  if (error && memories.length === 0) {
    return (
      <Box p={4} borderWidth="1px" borderRadius="md" borderColor="red.300" bg="red.50">
        <Heading size="md" color="red.500" mb={2}>Error Loading Memories</Heading>
        <Text>{error}</Text>
      </Box>
    );
  }
  ...
  return (
    <Box>
      ...
      <Tabs variant="enclosed" colorScheme="blue">
        ...
        <TabPanels>
          <TabPanel p={0} pt={4}>
            <MemoryList memories={filteredMemories} formatTimestamp={formatTimestamp} getAgentColor={getAgentColor} />
          </TabPanel>

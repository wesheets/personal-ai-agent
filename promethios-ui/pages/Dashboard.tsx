...
const Dashboard: React.FC = () => {
  ...
  const [messages, setMessages] = useState<UIMessage[]>([
    ...
    },
    {
      ...
      sender: 'agent',
      content:
        'I found several relevant articles about React state management. Would you like me to summarize the key points?',
  ...
  const [checkpoints, setCheckpoints] = useState<Checkpoint[]>([
    {
      ...
      description:
        'Confirm that the research phase for React state management is complete and sufficient for implementation.',
      type: 'hard',
      status: 'pending',
    ...
    },
    {
      ...
      description:
        'Approve the proposed implementation approach using Context API for the current feature.',
      type: 'soft',
      status: 'pending',
  ...
  const handleSendMessage = (content: string, threadParentId?: string) => {
    ...
    if (threadParentId) {
      setMessages((prev) =>
        prev.map((msg) => (msg.message_id === threadParentId ? { ...msg, isExpanded: true } : msg))
      );
    }
  ...
  const handleApproveCheckpoint = (checkpointId: string) => {
    setCheckpoints((prev) =>
      prev.map((cp) =>
        cp.id === checkpointId ? { ...cp, status: 'approved' as CheckpointStatus } : cp
      )
    );
  };
  ...
  const handleRejectCheckpoint = (checkpointId: string) => {
    setCheckpoints((prev) =>
      prev.map((cp) =>
        cp.id === checkpointId ? { ...cp, status: 'rejected' as CheckpointStatus } : cp
      )
    );
  };
    ...
    <Box p={4} bg={bgColor} minH="100vh">
      ...
      >
        ...
        <GridItem
          ...
          colSpan={1}
          overflowY="auto"
          display={isSandboxOpen ? 'block' : 'none'}
        >
          <AgentSandboxCard
            ...
            reflection="Planning next steps for implementation phase"
          />
          ...
          <AgentSandboxCard
            ...
            reflection="Comparing Context API and Redux performance characteristics"
          />
...
export const useCount = () => {
  return useContext(CountContext);
        ...
        <Drawer isOpen={isSandboxOpen} placement="right" onClose={onToggleSandbox} size="md">
          <DrawerOverlay />
          <DrawerContent>
            <DrawerCloseButton />
...
export const useCount = () => {
  return useContext(CountContext);
        ...
        <GridItem rowSpan={1} colSpan={1}>
          <InputUI onSendMessage={handleInputWithFiles} placeholder="Type a message..." />
        </GridItem>
      </Grid>
    </Box>
  );

...
export class CheckpointService {
  ...
  getCheckpointsForInstruction(instruction_id: string): Checkpoint[] {
    return Array.from(this.checkpoints.values())
      .filter(checkpoint => checkpoint.instruction_id === instruction_id);
  }
  ...
  areAllRequiredCheckpointsApproved(instruction: Instruction): boolean {
    ...
    return requiredCheckpointTags.every((tag: string) => 
      instructionCheckpoints.some(checkpoint => 
        checkpoint.tag === tag && checkpoint.status === 'approved'
      )
    );
  }
  ...
  private generateCheckpointId(): string {
    return 'chk_' + Math.random().toString(36).substring(2, 15) + 

    ...
    return (
      <div className="p-6 bg-white rounded-lg shadow-md">
        <div className="flex flex-col items-center justify-center h-64">
          ...
          <p className="text-gray-500 text-sm mt-1">Try interacting with agents to generate memory entries</p>
        </div>
      </div>
    );
  }
          ...
          <tbody>
            {memories.map((memory, index) => (
              <tr key={memory.id || index} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                <td className="py-2 px-4 font-medium">{memory.agent_id || memory.agentId || 'Unknown'}</td>
                <td className="py-2 px-4">
                  <span className={`inline-block px-2 py-1 rounded text-xs font-medium ${
                    memory.type === 'task' ? 'bg-blue-100 text-blue-800' : 
                    memory.type === 'delegation' ? 'bg-purple-100 text-purple-800' : 
                    memory.type === 'reflection' ? 'bg-green-100 text-green-800' : 

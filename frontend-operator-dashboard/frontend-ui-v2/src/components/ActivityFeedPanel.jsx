...
const mockActivities = [
  ...
  },
  {
    ...
    type: 'agent_response',
    content:
      "I'll help you analyze the dataset. Let me break it down into steps: 1) Data cleaning, 2) Exploratory analysis, 3) Visualization, 4) Insights generation.",
  ...
  },
  {
    ...
    type: 'agent_response',
    content:
      "I'll deploy the latest version to production. First, I'll run tests, then prepare the deployment package, and finally push to the production environment.",
...
const ActivityFeedPanel = () => {
  ...
  const renderActivityItem = (activity) => {
    ...
    return (
      <Box key={activity?.id ?? Math.random()} w="100%" mb={4}>
        <Flex justify={isUser ? 'flex-end' : 'flex-start'}>
          <HStack spacing={2} align="flex-start" maxW="80%">
            {!isUser && (
              <Avatar size="sm" bg={avatarBg} icon={<Icon as={avatarIcon} color="white" />} />
            )}
            ...
            {isUser && <Avatar size="sm" bg="blue.500" icon={<Icon as={FiUser} color="white" />} />}
          </HStack>
        </Flex>
      </Box>
    );
  ...
  return (
    <Flex direction="column" h="calc(100vh - 80px)" maxW="800px" mx="auto" position="relative">
      {/* Header */}
      <Flex

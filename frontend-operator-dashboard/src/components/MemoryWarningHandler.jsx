...
const MemoryWarningHandler = () => {
  ...
  const { memoryFallback, setMemoryFallbackState } = useStatus();
  ...
  return (
    <Box
      ...
      bg={
        memoryFallback.active ? (colorMode === 'light' ? 'yellow.50' : 'yellow.900') : 'transparent'
      }
      mb={4}
    >
            ...
            <HStack justifyContent="space-between">
              ...
              <Text fontSize="sm">
                {memoryFallback.timestamp
                  ? new Date(memoryFallback.timestamp).toLocaleTimeString()
                  : 'Unknown'}
              </Text>
            </HStack>

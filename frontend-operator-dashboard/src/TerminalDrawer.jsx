...
export default function TerminalDrawer({ open, onClose, payload, memory, logs }) {
  return (
    <div
      className={`fixed top-0 right-0 h-full w-[30rem] bg-black text-green-400 p-4 shadow-lg transition-transform z-50 ${open ? 'translate-x-0' : 'translate-x-full'}`}
    >
      <button onClick={onClose} className="text-white absolute top-2 right-2 text-xl">
        ✕
      </button>

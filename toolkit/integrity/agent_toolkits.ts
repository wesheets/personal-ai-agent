...
import { runAllValidators, checkMainRouter, checkChakraProvider, checkAuthProvider, checkDashboardPages, checkImportPaths, checkNoGitFolders, runViteDryBuild } from './validators';
 ...
 */
export async function verifyFolderIntegrity(): Promise<{success: boolean, message: string, details?: any}> {
  ...
  try {
    ...
    for (const pattern of legacyPatterns) {
      try {
        const result = execSync(`find ${UI_ROOT} -name "${pattern}" | grep -v "node_modules"`, { stdio: 'pipe' }).toString().trim();
        if (result) {
 ...
 */
export async function validateUIStructure(): Promise<{success: boolean, message: string, details: any}> {
  ...
  try {
    ...
    return {
      success,
      message: success 
        ? 'UI structure validation passed' 
        : 'UI structure validation failed',
      details: {
        router: routerCheck,
        chakra: chakraCheck,
        auth: authCheck
 ...
 */
export async function reportFailureDetails(): Promise<{success: boolean, message: string, reportPath?: string}> {
 ...
 */
export async function vercelRollbackLastGood(): Promise<{success: boolean, message: string, commit?: string}> {

 ...
 */
export async function installPrepareCommitMsgHook(): Promise<{success: boolean, message: string}> {
  try {
    // Create hooks directory if it doesn't exist
    if (!fs.existsSync(GIT_HOOKS_DIR)) {
      fs.mkdirSync(GIT_HOOKS_DIR, { recursive: true });
 ...
 */
export async function installAllCommitMessageHooks(): Promise<{success: boolean, message: string}> {
  try {

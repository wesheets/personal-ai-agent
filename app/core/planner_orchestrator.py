...
class PlannerOrchestrator:
    ...
    Integrated with Task State Manager for persistent memory-driven state management.
    """
    ...
    def __init__(self):
        ...
        self.task_state_manager = get_task_state_manager(self.vector_memory)
        ...
        try:
            ...
            if existing_tasks:
                ...
                if progress["status"] == "complete":
                    logger.info(f"Goal {goal_id} is already complete, returning previous results")
                    
                    # Retrieve the final result from memory
                    final_result = self._retrieve_goal_result_from_memory(goal_id)
                    
                    if final_result:
                        return final_result
                    
                    # If we couldn't retrieve the result, create a basic one
                    return {
                        "status": "success",
                        "goal_id": goal_id,
                        "message": "Goal was previously completed",
                        "completion_percentage": 100,
                        "timestamp": datetime.now().isoformat()
                    }
                
                # Resume the goal from where it left off
                logger.info(f"Resuming goal {goal_id} with progress: {progress['completion_percentage']}%")
                
                # Decompose the goal into subtasks if needed
                if not self.active_goals[goal_id].get("subtasks"):
                ...
                completed_subtask_ids = set(
                    task["subtask_id"] for task in existing_tasks 
                    if task["status"] == "complete"
                )
                
                remaining_subtasks = [
                    subtask for subtask in self.active_goals[goal_id]["subtasks"]
                    if subtask["id"] not in completed_subtask_ids
                ]
                ...
                for failed_task in failed_tasks:
                    subtask_id = failed_task["subtask_id"]
                    retry_count = failed_task.get("retry_count", 0)
                    max_retries = self.planner_config.get("max_retries", 3)
                    
                    if retry_count < max_retries:
                        # Find the corresponding subtask
                        for subtask in self.active_goals[goal_id]["subtasks"]:
                            if subtask["id"] == subtask_id:
                                # Add it to remaining subtasks if not already there
                                if subtask not in remaining_subtasks:
                                    logger.info(f"Adding failed subtask {subtask_id} for retry (attempt {retry_count + 1})")
                                    remaining_subtasks.append(subtask)
                                break
                
                # Execute the remaining subtasks
                if remaining_subtasks:
                    logger.info(f"Executing {len(remaining_subtasks)} remaining subtasks for goal {goal_id}")
                    results = self._execute_subtasks(remaining_subtasks, goal_id)
                    ...
                    logger.info(f"No remaining subtasks for goal {goal_id}")
                    
                    # Compile results from existing tasks
                    all_results = []
                    ...
                    # Compile the final results
                    final_result = self._compile_results(goal, all_results)
    ...
    def _assign_agents(self, subtasks: List[Dict[str, Any]], goal_id: str) -> List[Dict[str, Any]]:
        ...
        for subtask in subtasks:
            ...
            # Check if this subtask already has a task state
            existing_task_state = self.task_state_manager.get_task_state(subtask_id)
            
            if existing_task_state:
                # If the task is already complete, use the existing assignment
                if existing_task_state["status"] == "complete":
                    logger.info(f"Subtask {subtask_id} is already complete, using existing assignment")
                    
                    assigned_subtask = subtask.copy()
                    assigned_subtask["assigned_agent"] = existing_task_state["assigned_agent"]
                    assigned_subtask["assignment_time"] = existing_task_state["last_update"]
                    assigned_subtask["status"] = "complete"
                    
                    assigned_subtasks.append(assigned_subtask)
                    continue
                
                # If the task is in progress or failed but can be retried, use the existing assignment
                if existing_task_state["status"] in ["in_progress", "failed", "blocked"]:
                    # For failed tasks, check if they can be retried
                    if existing_task_state["status"] == "failed":
                        retry_count = existing_task_state.get("retry_count", 0)
                        max_retries = self.planner_config.get("max_retries", 3)
                        
                        if retry_count >= max_retries:
                            logger.info(f"Subtask {subtask_id} has failed and exceeded max retries, skipping")
                            continue
                    
                    logger.info(f"Subtask {subtask_id} is {existing_task_state['status']}, using existing assignment")
                    
                    assigned_subtask = subtask.copy()
                    assigned_subtask["assigned_agent"] = existing_task_state["assigned_agent"]
                    assigned_subtask["assignment_time"] = existing_task_state["last_update"]
                    assigned_subtask["status"] = existing_task_state["status"]
            ...
            # Create a task state for this subtask
    ...
    def _sequence_subtasks(self, subtasks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        sequenced_subtasks = [subtask_map[subtask_id] for subtask_id in sequenced_ids if subtask_id in subtask_map]
    ...
    def _execute_subtasks(self, subtasks: List[Dict[str, Any]], goal_id: str) -> List[Dict[str, Any]]:
        ...
        for subtask in subtasks:
            ...
            # Check if this subtask is already complete
            existing_task_state = self.task_state_manager.get_task_state(subtask_id)
            
            if existing_task_state and existing_task_state["status"] == "complete":
                logger.info(f"Subtask {subtask_id} is already complete, skipping execution")
                ...
                results.append({
                    ...
                    "result": {
                        "status": "success",
                        "output_summary": existing_task_state.get("output_summary", "Task was previously completed")
                    },
                    "execution_start_time": existing_task_state.get("created_at", ""),
                    "execution_end_time": existing_task_state.get("last_update", "")
                })
            ...
            try:
                ...
                task_status = "complete" if result.get("status") == "success" else "failed"
                output_summary = result.get("result", {}).get("output", "")
    ...
    def _should_escalate(self, subtask: Dict[str, Any], result: Dict[str, Any]) -> bool:
        ...
        if task_state:
            ...
            max_retries = escalation_policy.get("max_retry_attempts", 3)
            if retry_count >= max_retries:
                return True
    ...
    def _retrieve_goal_result_from_memory(self, goal_id: str) -> Optional[Dict[str, Any]]:
        """
        Retrieve the final result of a goal from memory.
        
        Args:
            goal_id: ID of the goal
            
        Returns:
            Goal result if found, None otherwise
        """
        try:
            # Search for the goal result in memory
            memories = self.vector_memory.search(
                query=f"goal_id:{goal_id}",
                metadata_filter={"entry_type": "goal_result", "status": "goal_complete"},
                limit=1
            )
            
            if memories and len(memories) > 0:
                # Parse the goal result from the memory
                memory_data = json.loads(memories[0]["text"])
                return memory_data.get("data", {})
        
        except Exception as e:
            logger.error(f"Error retrieving goal result from memory: {str(e)}")
    ...
    def get_goal_status(self, goal_id: str) -> Dict[str, Any]:
        ...
        # Check task state manager for goal progress
        goal_progress = self.task_state_manager.get_goal_progress(goal_id)
        
        if goal_progress["total_tasks"] > 0:
            ...
            try:
                ...
                if memories:
                    memory_data = json.loads(memories[0]["text"])
                    goal = memory_data.get("data", {})
                    
                    return {
                        "goal": goal,
                        "status": goal_progress["status"],
                        "completion_percentage": goal_progress["completion_percentage"],
                        "total_tasks": goal_progress["total_tasks"],
                        "completed_tasks": goal_progress["completed"],
                        "failed_tasks": goal_progress["failed"],
                        "timestamp": datetime.now().isoformat()
                    }
        ...
        # If not found in active goals or task state manager, try to retrieve from memory
        try:
            memories = self.vector_memory.search(
                query=f"goal_id:{goal_id}",
                metadata_filter={"entry_type": "goal"},
    ...
    def resume_goal(self, goal_id: str) -> Dict[str, Any]:
        ...
        if goal_progress["total_tasks"] == 0:
            return {
                "status": "not_found",
                "goal_id": goal_id,
                "message": "Goal not found in task state manager",
                "timestamp": datetime.now().isoformat()
            }
        
        if goal_progress["status"] == "complete":
            return {
        ...
        try:
            ...
            # Process the goal
            logger.info(f"Resuming goal {goal_id}")
            result = self.process_goal(goal)
            
            return {
                "status": "success",
                "goal_id": goal_id,
                "message": "Goal resumed and processed",
                "result": result,
                "timestamp": datetime.now().isoformat()
            }
    ...
    def replay_goal_history(self, goal_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get all task states related to the goal
            task_states = self.task_state_manager.get_goal_tasks(goal_id)
            
            if not memories and not task_states:
                return {
                    "status": "not_found",
                    "goal_id": goal_id,
                    "message": "Goal not found in memory or task state manager",
                    "timestamp": datetime.now().isoformat()
                }
            ...
            for task_state in task_states:
                events.append({
                    ...
                    "timestamp": task_state.get("last_update", ""),
                    "data": task_state
                })
    ...
    def get_stalled_goals(self, hours_threshold: int = 24) -> List[Dict[str, Any]]:
        """
        Get all goals that have been stalled for a certain period.
        
        Args:
            hours_threshold: Number of hours to consider a goal stalled
        ...
        # Get stalled tasks from task state manager
        stalled_tasks = self.task_state_manager.get_stalled_tasks(hours_threshold)
        
        # Group tasks by goal ID
        goal_tasks = {}
        ...
        # Get goal information for each stalled goal
...
def get_stalled_goals(hours_threshold: int = 24) -> List[Dict[str, Any]]:
    """
    Get all goals that have been stalled for a certain period.
    ...
    """
    orchestrator = get_planner_orchestrator()
    return orchestrator.get_stalled_goals(hours_threshold)

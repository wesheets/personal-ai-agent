...
# Import schemas
from app.schemas.reflection_schemas import ReflectionScanRequest, ReflectionScanResponse, ReflectionNode
...
class ReflectionScanner:
    ...
    def trigger_deep_scan(self, request: ReflectionScanRequest) -> ReflectionScanResponse:
        """
        Trigger a full system reflection deep scan.
        
        Args:
            request: ReflectionScanRequest object containing scan parameters
            
        Returns:
            ReflectionScanResponse object containing scan results
        """
        ...
        reflection_nodes: List[ReflectionNode] = []
        ...
        if request.filters:
            reflection_nodes = self._apply_filters(reflection_nodes, request.filters)
        
        # Generate scan result
        scan_result = ReflectionScanResponse(
            scan_id=self.scan_id,
            timestamp=self.scan_timestamp,
            status="completed",
            parameters=request.dict(), # Store original request parameters
    ...
    def _scan_agents_for_reflection(self, agents: List[Dict[str, Any]]) -> List[ReflectionNode]:
        """
        ...
        for agent in agents:
            ...
            if "name" in agent and "reflection" in agent["name"].lower(): is_reflection_agent = True
            if "role" in agent and "reflection" in agent["role"].lower(): is_reflection_agent = True
            if "capabilities" in agent and any("reflection" in cap.lower() for cap in agent["capabilities"]): is_reflection_agent = True
    ...
    def _scan_modules_for_reflection(self, modules: List[Dict[str, Any]]) -> List[ReflectionNode]:
        """
        ...
        for module in modules:
            ...
            if "name" in module and "reflection" in module["name"].lower(): is_reflection_module = True
            if "category" in module and "reflection" in module["category"].lower(): is_reflection_module = True
    ...
    def _scan_schemas_for_reflection(self, schemas: List[Dict[str, Any]]) -> List[ReflectionNode]:
        """
        ...
        for schema in schemas:
            ...
            if "name" in schema and "reflection" in schema["name"].lower(): is_reflection_schema = True
            if "category" in schema and "reflection" in schema["category"].lower(): is_reflection_schema = True
    ...
    def _apply_filters(self, nodes: List[ReflectionNode], filters: Dict[str, Any]) -> List[ReflectionNode]:
        """
        ...
        if "source_type" in filters:
            ...
            if isinstance(source_types, str): source_types = [source_types]
        ...
        if "reflection_type" in filters:
            ...
            if isinstance(reflection_types, str): reflection_types = [reflection_types]
        ...
        if "memory_tag" in filters:
            ...
            if isinstance(memory_tags, str): memory_tags = [memory_tags]
            filtered_nodes = [
                node for node in filtered_nodes 
                if node.metadata and "memory_tag" in node.metadata and node.metadata["memory_tag"] in memory_tags
            ]
...
def trigger_scan_deep(request: ReflectionScanRequest) -> ReflectionScanResponse:
    """
    Trigger a full system reflection deep scan.
    ...
    Returns:
        ReflectionScanResponse object containing scan results
    """
    # Re-initialize scanner for each request to get a new scan_id and timestamp
    current_scanner = ReflectionScanner()
    return current_scanner.trigger_deep_scan(request)

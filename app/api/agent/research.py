...
async def execute(
    ...
    """
    Execute the Research agent with the given input
    ...
    """
    try:
        # Load the research prompt chain
        prompt_chain = prompt_manager.get_prompt_chain("research")
        ...
        raise Exception(f"Error executing research agent: {str(e)}")
...
async def process_research_request(
    ...
    """
    Process a request using the Research agent
    """
    try:
...
async def get_research_history(limit: int = 10, db=Depends(get_database)):
    """
    Get history of research agent interactions
    """
    try:
        # Query the memory for research agent interactions

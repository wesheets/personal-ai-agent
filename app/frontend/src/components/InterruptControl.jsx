...
const InterruptControl = () => {
  ...
  useEffect(() => {
    const fetchSystemState = async () => {
      try {
        ...
        setSystemState({
          executionMode: controlModeData.mode || 'auto',
  ...
  const handleModeChange = async (mode) => {
    try {
      ...
      setSystemState(prev => ({ ...prev, executionMode: mode }));
    } catch (err) {
      console.error(`Error changing execution mode to ${mode}:`, err);
  ...
  const handleTaskRedirect = async (taskId, targetAgent) => {
    ...
    try {
      ...
      setError(`Failed to redirect task ${taskId}`);
    }
  };
  ...
  return (
    <Box>
      <Box mb={6} p={4} borderWidth="1px" borderRadius="lg" bg={bgColor} borderColor={borderColor}>
        ...
        <Flex gap={3}>
          <Button colorScheme={systemState.executionMode === 'auto' ? 'blue' : 'gray'} onClick={() => handleModeChange('auto')} flex="1">Auto Mode</Button>
          <Button colorScheme={systemState.executionMode === 'manual' ? 'blue' : 'gray'} onClick={() => handleModeChange('manual')} flex="1">Manual Mode</Button>
          <Button colorScheme={systemState.executionMode === 'paused' ? 'yellow' : 'gray'} onClick={() => handleModeChange('paused')} flex="1">Pause All</Button>
        </Flex>
      </Box>
          ...
          <VStack spacing={4} align="stretch" divider={<Divider />}>
            {activeTasks.map((task) => (
              <Box key={task.task_id} p={3} borderWidth="1px" borderRadius="md">
                ...
                <Text fontSize="sm" mb={3}>Agent: {task.assigned_agent || 'Unassigned'}</Text>
                <Flex gap={2} wrap="wrap">
                  <Button size="sm" colorScheme="yellow" onClick={() => handleTaskAction(task.task_id, task.status === 'in_progress' ? 'kill' : 'restart')} isDisabled={task.status === 'completed'}>
                    {task.status === 'in_progress' ? 'Kill' : 'Restart'}
                  </Button>
                  <Select size="sm" placeholder="Redirect to..." onChange={(e) => handleTaskRedirect(task.task_id, e.target.value)} isDisabled={task.status === 'completed'} maxW="200px">
                    {systemState.activeAgents.map((agent) => (
                      <option key={agent} value={agent}>{agent}</option>
                    ))}
                  </Select>
                  {systemState.executionMode === 'manual' && (
                    <Button size="sm" colorScheme="blue" onClick={() => openPromptEditor(task)} isDisabled={task.status === 'completed'}>
                      Edit Prompt
                    </Button>
                  )}
                </Flex>
